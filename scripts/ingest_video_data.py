...
def uploadVideo(filepath, description):
    ...
    cursor.execute(
        "INSERT INTO video_metadata (filename, description, duration, dark_setting, nbr_people, fps) VALUES (?, ?, ?, ?, ?, ?)",
        (filename, description, duration, video_dark_setting, nbr_people, fps),
    )
    sqliteConnection.commit()
